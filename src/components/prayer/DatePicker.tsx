...
export function DatePicker({ onSelectDate, selected }: DatePickerProps) {
  const [open, setOpen] = React.useState(false);
  const [date, setDate] = React.useState<Date | undefined>(selected); // لا نعطي قيمة افتراضية اليوم

  const currentYear = new Date().getFullYear();

  // مزامنة state مع prop selected
  React.useEffect(() => {
    setDate(selected);
  }, [selected]);
